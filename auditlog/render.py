"""
Rendering utilities for auditlog changes display.
"""
...
def render_changes(log_entry):
    """
    Render LogEntry changes as HTML tables.
    
    :param log_entry: LogEntry instance
    :return: HTML string with formatted changes
    """
...
def _render_m2m_changes(log_entry, m2m_changes):
    """Render M2M relationship changes as a table."""
    rows = []
...
def _format_row(*values):
    ...
    return format_html(
        "".join(["<tr>", "<td>{}</td>" * len(values), "</tr>"]), 
        *values
    )


def _get_field_verbose_name(log_entry, field_name):
